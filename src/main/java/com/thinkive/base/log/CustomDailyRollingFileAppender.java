...
public class CustomDailyRollingFileAppender extends FileAppender
{
    ...
    public void activateOptions()
    {
        ...
        if ( datePattern != null && fileName != null )
        {
            ...
            scheduledFilename = getFileName(new Date(file.lastModified()));
    ...
    void rollOver() throws IOException
    {
        ...
        String datedFilename = getFileName(now);
        ...
        if ( files.size() >= maxBackupIndex )
        {
            ...
            for (ModifiedTimeSortableFile mFile : files)
            {
                if ( index >= diff )
                    break;
    ...
    protected String getFileName(Date date)
    {
        ...
        return fileName.substring(0, index) + sdf.format(date) + fileName.substring(index);
    }
    ...
    private List<ModifiedTimeSortableFile> getAllFiles()
    {
        ...
        FilenameFilter filter = new FilenameFilter()
        {
            ...
            public boolean accept(File dir, String name)
            {
                ...
                {
                    int indexStart = directoryName.length() + 1;
                    int indexEnd = fileName.lastIndexOf(".");
                    String localFile = null;
                    if ( indexEnd > indexStart )
                    {
                        localFile = fileName.substring(indexStart, indexEnd);
                    }
                    else
                    {
                        localFile = fileName.substring(indexStart);
                    }
